...
export const CardSelect = observer(({onEnd}) => {
    ...
    if(selectedCardsState.step === 2) {
        renderedItems = items.filter(i=>selectedCardsState.selectedCards.includes(i.id)).map(i=>({...i, closable: true, selectable: true}));
    }
    ...
    return (
        <div>
            {thoughtVisible && <ProtagonistThought onClick={()=>setThoughtVisible(false)} text={selectedCardsState.text}/>}
